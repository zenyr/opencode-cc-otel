...
- `packages/adapters` owns sink implementations and delivery behavior: retry/backoff, durability, fanout, OTEL/OTLP JSON envelopes, writer/transport policy.
...
- adapter stack can compose 1P HTTP batch sink + durable queue replay + 2P OTEL JSON or OTLP JSON export.
- OTEL-style export is implemented as Claude-compatible log/metric JSON envelopes or OTLP JSON payloads, not native OpenTelemetry SDK wiring.
...
- 2P config keeps these concerns separate: `sink = "otel-json" | "otlp-json"`, `transport = "file" | "console" | "http"`.
